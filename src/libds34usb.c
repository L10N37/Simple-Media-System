/* DS3/DS4-over-USB EE client library ( SIF RPC to ds34usb.irx ). Vendored from
 * Open PS2 Loader ( github.com/ps2homebrew/Open-PS2-Loader, modules/ds34usb/ee/ ),
 * (c) the OPL contributors, Academic Free License v3.0 -- compatible with SMS
 * ( AFL v2.0 ). SMS-local change: ds34usb_init()'s RPC bind spin is bounded. */
#include <kernel.h>
#include <sifrpc.h>
#include <string.h>
...
int ds34usb_init()
{
    int retries = 100;   /* SMS: bound the bind spin ( nopdelay is ~1M nops each ) so a missing RPC server costs ~1s, not minutes. The module is loaded before this runs, so its server registers within a couple of iterations; the caller (main.c) only calls this when SMS_IOPStartDS34USB confirmed the load, so this cap is just a backstop. */
    ...
    do {
        ...
        nopdelay();
    } while (!ds34usb.server && --retries);

    if (!ds34usb.server)   /* server never registered -> give up, don't hang */
        return 0;
