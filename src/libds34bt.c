...
int ds34bt_init()
{
    int retries = 100;   /* SMS: bound the bind spin ( nopdelay is ~1M nops each ) so a missing RPC server costs ~1s, not minutes. Caller only invokes this after SMS_IOPStartDS34BT confirmed the module loaded, so this is just a backstop. */
    ...
    do {
        ...
        nopdelay();
    } while (!ds34bt.server && --retries);

    if (!ds34bt.server)   /* server never registered -> give up, don't hang */
        return 0;
