...
/* SMS: escape count for the driver's unbounded hardware-status spins. Large enough that a
 * healthy SPU2 always settles inside it ( these bits clear in well under a microsecond ),
 * small enough that a stuck bit costs a dropped buffer instead of the console. */
#define SMS_SPIN_LIMIT 1000000u
...
int TransInterrupt(void *data)
{
	...
	if((intr->mode & 0x100) == 0)
	{
		...
		/* SMS: BOUNDED. Both of these were `while (...);` with no escape, INSIDE an
		 * interrupt handler -- so a status bit that never settles does not glitch the
		 * audio, it hangs the IOP with interrupts disabled and takes the whole console
		 * with it. See tools/libsd/README-SMS.md for why that is reachable here. */
		{ u32 _g = SMS_SPIN_LIMIT; while((U16_REGISTER_READ(statx) & 0x80) == 0 && --_g); }

		U16_REGISTER_WRITEAND(SD_CORE_ATTR(core), ~SD_CORE_DMA);

		{ u32 _g = SMS_SPIN_LIMIT; while((U16_REGISTER_READ(SD_CORE_ATTR(core)) & 0x30) != 0 && --_g); }
