...
{
/* SMS: zero-initialised, NOT left as raw stack. _ist only fills this when the server's
 * header both arrives and FITS ( the wire-size clamp in udprdma.c ); an undersized
 * header from a version-skewed server writes msg_type but NOT result, and an oversized
 * one is drained without writing anything -- either way the fields below get READ. On
 * garbage stack that returned a bogus byte count under a valid-looking msg_type
 * ( silent wrong data to the EE ); on zeros it fails the msg_type check / reads 0 bytes
...
int udpfs_core_init(void)
{
    ...
    /* SMS: create the UDPRDMA socket ONCE and keep it for the module's lifetime.
     *
     * Upstream created a socket here and destroyed it on a failed discovery. That is
     * only safe because upstream calls this exactly once, at module load. SMS retries
     * (lazy connect on open/dopen, plus the EE-side probe loop), and a
     * create-per-attempt is UNRECOVERABLE here: ministack's udp_bind has only
     * UDP_MAX_PORTS == 4 slots and hands out the first with port_src == 0
     * (ministack_udp.c:17-24), while udprdma_destroy CANNOT give one back -- it says so
     * itself, "UDP socket cleanup not implemented in ministack" (udprdma.c:397), and
     * only clears socket->udp_socket. So every failed attempt would burn a slot; after
     * four, udp_bind returns NULL and udpfs: is dead for the rest of the boot -- the
     * exact "no device, nothing" failure the lazy connect exists to prevent, just via a
     * different mechanism.
     *
     * Re-discovering on the SAME socket is free and correct: udprdma_discover resets
     * state to STATE_DISCOVERING itself (udprdma.c:413) and requires no particular
     * prior state, so this recovers a STATE_DISCONNECTED link too. Keeping one socket
     * also avoids a second udp_bind on the same port: handle_rx_udp dispatches to the
     * FIRST port_src match, so a stale entry would swallow the reply and hand it to the
     * old socket's handler_arg -- discovery would then never complete on the new one. */
    if (g_socket == NULL) {
    ...
    if (ret != UDPRDMA_OK) {
        ...
        /* Deliberately NOT destroying the socket: its udp_bind slot can never be
         * reclaimed, and the socket is fully reusable for the next discovery. */
        return -1;
    }
