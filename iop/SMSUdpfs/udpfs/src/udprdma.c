            ...
            /* Handle payload if present */
            {
                ...
                if (payload_size > 0 && s->rx_buffer != NULL) {
                    if (base_hdr.seq_nr == s->rx_seq_nr_expected) {
                         ...
                         *
                         * SMS: hdr_size is WIRE-CONTROLLED -- hdr_word_count is a 4-bit
                         * field, so it reaches 60 bytes -- and upstream fed it unchecked
                         * to both reads. rx_hdr_buffer is the caller's buffer of exactly
                         * rx_hdr_size bytes (8, for udpfs_msg_result_reply_t) and dummy[]
                         * is UDPRDMA_MAX_APP_HDR (32), so a server sending a larger count
                         * -- version-skewed, buggy, or hostile on the LAN -- smashed the
                         * ioman thread's stack from the SMAP RX thread. rx_hdr_size exists
                         * for exactly this and was simply never read.
                         * Only fill the caller's buffer when the header genuinely fits;
                         * otherwise DRAIN in dummy-sized steps. Draining is not optional:
                         * these bytes are already in the FIFO, and leaving them would
                         * desync every following payload read. */
                        if (hdr_size > 0) {
                            uint8_t  dummy[UDPRDMA_MAX_APP_HDR];
                            uint32_t left = hdr_size;
                            ...
                            while (left > 0) {   /* no buffer, oversized header, or header already taken */
                                uint32_t n = left > sizeof(dummy) ? sizeof(dummy) : left;
                                smap_fifo_read(0x30, dummy, n);
                                left -= n;
                            }
...
int udprdma_discover(udprdma_socket_t *socket, uint32_t timeout_ms)
{
    ...
    /* SMS: sanitize the per-session RX state before (re-)discovering.
     *
     * Upstream reached discovery only on a socket freshly memset by udprdma_create.
     * SMS keeps ONE socket for the module lifetime and re-discovers on it (see the note
     * in udpfs_core.c: ministack's udp_bind has 4 slots and udprdma_destroy cannot give
     * one back, so create-per-retry is fatal), which means a re-discovery would
     * otherwise inherit the DEAD session's RX state. Two ways that bites, both real:
     *
     *  - Dangling IST target. If udprdma_send exhausts its retries inside
     *    _recv_with_result, it returns -EIO WITHOUT ever calling udprdma_recv, leaving
     *    rx_hdr_buffer/rx_buffer pointing at that function's now-popped STACK LOCALS.
     *    _ist gates only on `s->rx_buffer != NULL`, so a late Go-Back-N retransmit
     *    arriving during the 5s discover window would smap_fifo_read straight into dead
     *    stack -- from interrupt context.
     *  - Stale completion bit. A late DATA packet can set EF_RX_FIN after discovery's
     *    WEF_CLEAR wait has already passed. udprdma_send deliberately preserves
...
/* The SEQUENCE NUMBERS must go back to 0 too. UDPRDMA.md: both sides start a data
 * connection at seq 0, and the server does exactly that on every INFORM. Carrying a
 * dead session's counters across a re-discovery makes the connection look established
 * ( is_connected() == 1, the drive shows as mounted ) while every single request is then
 * outside the server's Go-Back-N window -- so udpfs: reconnects and never transfers
 * another byte. udprdma_create's memset used to supply this; keeping one socket for the
 * module lifetime means doing it here instead. */
    socket->tx_seq_nr        = 0;
...
/* SMS: disarm the RX pointers as the session dies -- the same thing udprdma_recv already
 * does on its timeout path. A caller such as udpfs_core.c's _recv_with_result arms us
 * with pointers to its own STACK LOCALS and then returns -EIO the moment send fails,
 * WITHOUT ever calling udprdma_recv -- so nothing else disarms them and s->rx_hdr_buffer
